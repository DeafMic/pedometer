...
double calculateCompassBearing(double a0, double a1, double a2,
	...
	//double yawAngle = atan2((m2 * sin(rollAngle) - m1 * cos(rollAngle)),
	//		(m0 * cos(pitchAngle) + m1 * sin(pitchAngle) * sin(rollAngle)
	//				+ m2 * sin(pitchAngle) * cos(rollAngle)));
...
int CCONV SpatialDataHandler(CPhidgetSpatialHandle spatial, void *userptr, CPhidgetSpatial_SpatialEventDataHandle *data, int count)
{
	...
	double heading = calculateCompassBearing(data[i]->acceleration[0],data[i]->acceleration[1],data[i]->acceleration[2],data[i]->magneticField[0],data[i]->magneticField[1],data[i]->magneticField[2]);
